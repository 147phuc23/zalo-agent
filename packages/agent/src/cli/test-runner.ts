...
for (const testCase of cases) {
  ...
  let lastResult: HrAgentRunResult | null = null;
...
function checkAssertions(assertions: Assertion[], result: HrAgentRunResult): string[] {
  const failures: string[] = [];
  const response = result.assistantText.toLowerCase();

  const calledToolNames = new Set(
    result.steps.flatMap((step) =>
      (step.toolCalls as Array<{ toolName?: string }> ?? [])
        .map((c) => c.toolName)
        .filter(Boolean),
    ),
  );
...
function parseArgs(argv: string[]) {
  const args = { caseId: undefined as string | undefined, verbose: false };
