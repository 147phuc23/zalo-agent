...
// JD schema & system prompt from feature/jd-to-job-drafts
...
// CV schemas & prompt from feature/cv-to-profile
    ...
    async (job) => {
      ...
      try {
        ...
        if (doc.kind === "cv") {
          ...
          if (twentyRuntimeEnabled) {
            try {
              ...
              const loadedProfile = await recruitingClient.loadCandidateProfile({ externalUserId });
              const matches = await recruitingClient.computeJobMatchScores({ profile: loadedProfile });
        ...
        if (doc.kind === "jd") {
          ...
          const model = process.env.HR_AGENT_MODEL || "google/gemini-2.5-flash";
          const client = new OpenRouterAiClient();
          const response = await client.generate({
            model,
          ...
          await deps.repos.jobs.createDraft({
            ...
            fields: {
              ...
              company: companyName,
              locationSlugs: locationSlugs.length > 0 ? locationSlugs : ["remote"],
