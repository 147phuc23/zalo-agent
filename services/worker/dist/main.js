...
async function generateDraftReply(tenantId, conversationId, abortSignal) {
    ...
    if (cached && (now - cached.lastUpdated) < CACHE_TTL_MS) {
        ...
        const merged = Array.from(cachedMap.values()).sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
...
function parseDraftResponses(text) {
    ...
    return normalizeResponses(text
        .split(/\n+/)
...
async function appendAuditError(tenantId, conversationId, toolName, output) {
    const auditRow = await repos.audits.append({
        tenantId,
        conversationId,
        runId: null,
