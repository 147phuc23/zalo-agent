...
function printInboundMessage(externalUserId, text, styledOutput) {
    if (!styledOutput) {
...
function printSystemResponse(text, styledOutput) {
    if (!styledOutput) {
        console.log(text);
        return;
    }
...
function printDebugStep(index, step, styledOutput) {
    const lines = [
...
function debugJsonLines(label, value) {
    if (!value?.length)
        return [];
