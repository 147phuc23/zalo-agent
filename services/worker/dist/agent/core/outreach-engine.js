...
Reply in Vietnamese with friendly emojis.`;
export async function generateFollowUpMessage(candidateName, roleName, model = "openrouter/owl-alpha") {
...
export async function generateJobOutreachMessage(candidateName, jobTitle, company, model = "openrouter/owl-alpha") {
    const client = new OpenRouterAiClient();
    const response = await client.generate({
        model,
...
export async function generateUrgentOutreachMessage(candidateName, jobTitle, company, model = "openrouter/owl-alpha") {
    const client = new OpenRouterAiClient();
    const response = await client.generate({
        model,
...
export async function runPipelineFollowUpsCampaign(input) {
    ...
    const res = await input.db.query(`SELECT c.id, c.tenant_id, c.external_thread_id, c.contact_id, c.last_activity_at 
...
export async function runReverseMatchingCampaign(input) {
    ...
    const res = await input.db.query(`SELECT c.id, c.external_user_id, c.display_name 
     FROM contacts c 
     WHERE c.tenant_id = $1`, [input.tenantId]);
    let outreachCount = 0;
    for (const contactRow of res.rows) {
        ...
        if (match && match.score >= 8) {
            ...
            const convRes = await input.db.query(`SELECT id, external_thread_id FROM conversations WHERE contact_id = $1 LIMIT 1`, [contactRow.id]);
...
async function sendOutboundMessage(input) {
    ...
    await input.redisPublisher.publish("platform:sse", JSON.stringify({
    ...
    await input.messageSendQueue.add("message.send", {
        tenantId: input.tenantId,
        channel: "zalo",
...
export function startOutreachCampaignWorkers(input) {
    ...
    const followupWorker = new Worker("outreach.followup", async (job) => {
    ...
    const campaignWorker = new Worker("outreach.campaign", async (job) => {
        const data = job.data;
