...
export async function classifyIntent(messages, model = "openrouter/owl-alpha") {
...
export async function generateChitchatReply(messages, model = "openrouter/owl-alpha") {
    const client = new OpenRouterAiClient();
    const historyText = formatHistory(messages);
    const response = await client.generate({
        model,
