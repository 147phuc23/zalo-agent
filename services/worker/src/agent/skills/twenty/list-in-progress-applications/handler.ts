...
export function createTwentyListInProgressApplicationsTool() {
  return tool({
    description:
      "List active job applications for a candidate from Twenty `jobApplication` records (stages: applied, screening, interview). Joins `jobPosting` details when present.",
    parameters: z.object({
